...
// renvoie le nom de la case : e4 -> "e4
[[nodiscard]] inline std::string name(const int square) {
    std::string str;
    str += static_cast<char>('a' + square % 8);
    str += static_cast<char>('1' + square / 8);
    return str;
}
...
[[nodiscard]] constexpr bool light(int sq) noexcept {
    return (rank(sq) + file(sq)) % 2;
}

[[nodiscard]] constexpr bool dark(int sq) noexcept {
    return !light(sq);
}
...
template <Color C>
[[nodiscard]] constexpr bool is_on_seventh_rank(const int x) {
    return (C) ? rank(x) == 1 : rank(x) == 6;
}

/* Check if square 'x' is on 2nd rank */
template <Color C>
[[nodiscard]] constexpr bool is_on_second_rank(const int x) {
